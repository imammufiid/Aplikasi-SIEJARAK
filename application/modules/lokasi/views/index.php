...
<script type="text/javascript">
	$(document).ready(function() {
		var tabel = $('#tabel-data').DataTable({
			ajax: {
				url: "<?php echo base_url('lokasi/fetch') ?>",
				dataSrc: "data",
				"type": "POST",
				'dataType': 'json',
			},
		...
		$('#form-md-manage').submit(function(event) {
			$.post('<?php echo base_url('lokasi/save') ?>', $(this).serialize(), function(response, textStatus, xhr) {
				tabel.ajax.reload();
		...
		$('#form-md-delete').submit(function(event) {
			$.post('<?php echo base_url('lokasi/delete') ?>', $(this).serialize(), function(response, textStatus, xhr) {
				tabel.ajax.reload();
				if (response.status == true) {
					toastr.success(response.msg);
	...
	function dt_add(t) {
		$.get('<?php echo base_url('lokasi/modal/add') ?>').done(function(data) {
	...
	function dt_edit(t) {
		$.get('<?php echo base_url('/lokasi/modal/edit') ?>', {
			'id': $(t).attr('target-id')
		}).done(function(data) {
	...
	function dt_delete(t) {
		$.get('<?php echo base_url('lokasi/modal/delete') ?>', {
			'id': $(t).attr('target-id')
		}).done(function(data) {
